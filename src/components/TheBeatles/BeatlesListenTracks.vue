  ...
  <div class="content__items" ref="cont" @click="emitTrack(artist, id)">
    ...
    <div class="content__track">
      <div class="content__id">
        <div class="db">
          <h4 class="content__track-turnOnTrack" :class="{'active': artist.isActive}">{{ id }}</h4>
          <div class="img">
            <img v-lazy="img.url" alt="" class="track__img" v-for="(img, index) in artist.album.images" :key="index">
          </div>
        </div>
      ...
      <div class="content__nameTrack">
        <h4 class="content__nameTrack-songNames" v-if="name.length > 38" :class="{'active': artist.isActive}">
          {{ name.slice(0, 35) }}...
        </h4>
        <h4 class="content__nameTrack-songNames" v-else  :class="{'active': artist.isActive}">{{ name }}</h4>
        <div class="artists">
          <span
